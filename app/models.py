...
class Wallet(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status in ('active', 'inactive', 'blocked')", name="ck_wallets_status"
        ),
        CheckConstraint(
            "currency in ('USD', 'EUR', 'RUB', 'CNY')", name="ck_wallets_currency"
        ),
...
class Transaction(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "status in ('in_process', 'completed', 'failed', 'blocked')",
